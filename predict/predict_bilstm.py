...
def fit_bilstm(data_prepare, n_neurons=50, n_batch=72, n_epoch=100, loss='mae', optimizer='adam', repeats=1):
    ...
    for i in range(repeats):
        ...
        bilstm.add(Bidirectional(keras.layers.LSTM(n_neurons, input_shape=(train_X.shape[1], train_X.shape[2]))))
        bilstm.add(keras.layers.Dense(16))
        bilstm.add(keras.layers.LeakyReLU(alpha=0.3))
        # 输出层
        bilstm.add(keras.layers.Dense(train_y.shape[1]))
        bilstm.add(keras.layers.LeakyReLU(alpha=0.3))
        # 定义优化器
        #nadam = keras.optimizers.Nadam(lr=1e-3)
        bilstm.compile(optimizer='adam', loss='mse', metrics=['accuracy'])
        history = bilstm.fit(train_X, train_y, epochs=n_epoch, batch_size=n_batch, validation_data=(test_X, test_y),
...
def bilstm_predict(model, data_prepare):
...
def several_days_prediction_bilstm(day=5, n_in = 1, n_vars = 14, n_neuron = 5, n_batch = 16, n_epoch = 200, repeats = 10, n_train=5):
    df_old = g_data()
    ...
    df_new = pd.DataFrame({'date': a0_list, 'Singapore': a1_list, 'BDI': a2_list, 'WTI': a3_list, 'IMPORT': a4_list,
                           ...
                           'EXPORT': a10_list, 'Yield': a11_list, 'dollar': a12_list, 'f1': a13_list, 'f2': a14_list,
                           'f3': a15_list})
    df_old = df_old[
    ...
    data_prepare = prepare_data(dataset=df, n_in=n_in, n_vars=n_vars, n_out=day, n_train=-3)
    scaler, data, train_X, train_y, test_X, test_y, dataset = data_prepare
    model = fit_bilstm(data_prepare, n_neuron, n_batch, n_epoch, repeats=repeats)
    i1_list = []

    for i in range(repeats):
        i1_list.append(bilstm_predict(model[i], data_prepare)[0])

    inv_y = bilstm_predict(model[0], data_prepare)[1]
    ...
    df3.to_excel('short_prediction_bilstm.xlsx')



def five_day_bilstm(df):
    ...
    n_epoch = 200
    repeats = 5
    data_prepare = prepare_data(dataset=df, n_in=n_in, n_vars = n_vars, n_out=n_out, n_train=-3)
    scaler, data, train_X, train_y, test_X, test_y, dataset = data_prepare
    model = fit_bilstm(data_prepare, n_neuron, n_batch, n_epoch, repeats=repeats)
    i1_list = []
...
def five_day_predcition_bilstm(length = 10, start = 10):
    ...
    df2 = pd.DataFrame(
        {'date': date_list, 'price': price_list, 'p1': p1_list, 'p2': p2_list, 'p3': p3_list, 'p4': p4_list,
         'p5': p5_list})
    df2.to_excel('five_day_prediction_bilstm.xlsx')
